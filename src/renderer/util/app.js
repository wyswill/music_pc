export default {
	...
	data() {
		return {
			music_type_list: [ '个性推荐', '歌单', '主播电台', '排行榜', '歌手', '最新音乐', 'b', 'f' ],
			statu: ''
		};
	},
	methods: {
      ...
      ------------------*/
			...
			let arr = [ '#/', '#/sun_list', '#/zhuBo', '#/paiHangBang', '#/seaner', '#/newMusic' ];
			// 设置样式
	...
	async mounted() {
      ...
      ------------------*/
		this.seet_active();
		let status = await this.api.getUserStatus();
		this.statu = status.msg;
	},
	updated() {
		/* 设置选中样式 
    ------------------*/
